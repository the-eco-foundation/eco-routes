...
async function proveMainnetSettlementLayerStateOnHelix() {
  ...
  while (!provedSettlementState) {
    ...
    try {
      ...
      tx = await s.helixProverContract.proveSettlementLayerStatePriveleged(
        getBytes(hexlify(rlpEncodedBlockData)),
        networkIds.mainnet,
      )
      await tx.wait()
...
async function proveWorldStateBaseOnHelix(
  settlementBlockTag,
  settlementStateRoot,
  ...
  console.log('In proveWorldStateCannonBaseToHelix')
...
async function proveWorldStatesCannonL2L3(
  ...
  const endBatchBlockData = await proveWorldStateBaseOnHelix(
    settlementBlockTag,
    settlementWorldStateRoot,
    faultDisputeGameAddress,
    faultDisputeGameContract,
...
async function proveWorldStatesCannon(
  ...
  const endBatchBlockData = await proveWorldStateBaseOnOptimism(
    settlementBlockTag,
    settlementWorldStateRoot,
    faultDisputeGameAddress,
    faultDisputeGameContract,
