...
export async function getBatchSettled() {
  ...
  const approximateUnsettledGames = 70n // Initial Test on Mainnet gave 84
  ...
  while (gameIndex > 0) {
    ...
    if (faultDisputeGameResolvedEvents.length !== 0) {
      ...
      console.log('GetBatchSettled return data')
...
export async function getIntentsToProve(
  ...
  let startingBlockNumber = networks.optimism.inbox.deploymentBlock || 0n
  const inboxDeploymentBlock = networks.optimism.inbox.deploymentBlock
  ...
  for (const sourceChain of sourceChainConfig) {
    ...
    try {
      ...
      if (proveAll) {
        ...
        if (optimismProvenState.blockNumber > inboxDeploymentBlock) {
          sourceChainInfo.lastProvenBlock = optimismProvenState.blockNumber
          if (optimismProvenState.blockNumber < startingBlockNumber) {
            startingBlockNumber = optimismProvenState.blockNumber
          }
        } else {
          sourceChainInfo.lastProvenBlock = inboxDeploymentBlock
          scanAllIntentsForInbox = true
  ...
  console.log('startingBlockNumber: ', startingBlockNumber)
  console.log('settlementBlockNumber: ', settlementBlockNumber)
  // Get the event from the latest Block checking transaction hash
  const intentHashEvents = await s.optimismInboxContractSolver.queryFilter(
    s.optimismInboxContractSolver.getEvent('Fulfillment'),
...
async function proveMainnetSettlementLayerStateOnHelix() {
  console.log('In proveMainnetSettlementLayerStateOnHelix')
  ...
  while (!provedSettlementState) {
    ...
    // const l2SettlementBlock = await s.helixl1Block.number()
    // const l2SettlementBlockTag = toQuantity(l2SettlementBlock)
    // const l2block: Block = await s.baseProvider.send(
    //   'eth_getBlockByNumber',
    //   [l2SettlementBlockTag, false],
    // )
    // Get L2 Settlement Block from lastest L2 Block
    const l2block: Block = await s.baseProvider.send('eth_getBlockByNumber', [
      'latest',
      false,
    ])
    ...
    const l1SettlementBlock = await s.basel1Block.number()
    const l1SettlementBlockTag = toQuantity(l1SettlementBlock)
    const l1block: Block = await s.mainnetProvider.send(
      'eth_getBlockByNumber',
    ...
    // const l2MesagePasserProof = await s.baseProvider.send(
    //   'eth_getProof',
    //   [
    ...
    //   toBeArray(networks.base.proving.l1BlockSlotNumber),
    //   32,
    // )
    const l2BlockHashSlot = toBeHex(
      BigInt(networks.base.proving.l1BlockSlotNumber),
      32,
    )
    console.log(
    ...
    const l2l1BlockProof = await s.baseProvider.send('eth_getProof', [
      networks.base.proving.l1BlockAddress,
      [l2BlockHashSlot],
      l2SettlementBlockTag,
    ])
    ...
    const RLPEncodedl2l1BlockContractData =
      await s.baseProverContract.rlpEncodeDataLibList(l2l1BlockContractData)
    ...
    await s.helixProverContract.proveStorage(
    ...
    await s.helixProverContract.proveAccount(
      networks.base.proving.l1BlockAddress, // abi.encodePacked(L1_BLOCK_ADDRESS),  L1BlockOracle Address
    ...
    while (!finishedThisRound) {
      ...
      try {
        ...
        const l2SettlementBlockNow = await s.helixl1Block.number()
        console.log('l2SettlementBlock: ', l2SettlementBlock)
        ...
        tx = await s.helixProverContract.proveL1L3SettlementLayerState(
...
async function proveWorldStateOptimismOnHelix(
  settlementBlockTag,
  settlementStateRoot,
  faultDisputeGameAddress,
  faultDisputeGameContract,
  ...
  console.log('In proveWorldStateOptimismOnHelix')
  ...
  const faultDisputeGameId = await s.helixProverContract.pack(
  ...
  const { gameProxy_ } = await s.helixProverContract.unpack(
  ...
  await s.helixProverContract.proveStorage(
    faultDisputeGameResolvedStorageSlot,
    await s.helixProverContract.assembleGameStatusStorage(
      faultDisputeGameCreatedAt,
  ...
  try {
    const proveWorldStateCannonTx =
      await s.helixProverContract.proveWorldStateCannon(
        networkIds.optimism,
    ...
    console.log('ProveWorldStateCannon Base to Helix')
    ...
    if (e.data && s.helixProverContract) {
      ...
      console.log(`Error in ProveWorldStateCannon helix:`, e.shortMessage)
    } else {
      console.log(`Error in ProveWorldStateCannon helix:`, e)
    }
  }
}

async function proveWorldStateOptimismOnBase(
  settlementBlockTag,
  settlementStateRoot,
  faultDisputeGameAddress,
  faultDisputeGameContract,
  ...
  console.log('In proveWorldStateCannonBaseToBase')
  // For more information on how DisputeGameFactory utility functions, see the following code
  // https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/lib/LibUDT.sol#L82
  // get the endBatchBlockData
...
async function proveWorldStatesCannonL2L3(
  ...
  const { l1SettlementBlockTag, settlementWorldStateRoot } =
    await proveMainnetSettlementLayerStateOnHelix() // Prove the Mainnet Settlement Layer State

  const endBatchBlockData = await proveWorldStateOptimismOnHelix(
    l1SettlementBlockTag,
    settlementWorldStateRoot,
    faultDisputeGameAddress,
    faultDisputeGameContract,
...
async function proveWorldStatesCannon(
  ...
  const endBatchBlockData = await proveWorldStateOptimismOnBase(
    settlementBlockTag,
    settlementWorldStateRoot,
    faultDisputeGameAddress,
    faultDisputeGameContract,
...
async function main() {
  // const { l1SettlementBlockTag, settlementWorldStateRoot } =
  //   await proveMainnetSettlementLayerStateOnHelix()
  // console.log('l1SettlementBlockTag: ', l1SettlementBlockTag)
  ...
  try {
    console.log('In Main')
    console.log('Batch Settle of OP Mainnet')
    // Get the latest Batch Settled for OP Mainnet
    const {
      blockNumber,
      gameIndex,
      faultDisputeGameAddress,
