...
import { networks, intent, actors } from '../../config/mainnet/config'
...
async function main() {
  ...
  intentCreator = s.baseIntentCreator
  solver = s.optimismSolver
  console.log(`From ${sourceNetwork.network} to ${destinationNetwork.network}:`)
  await hyperproveInstant()
  // switch networks
  ;[sourceNetwork, destinationNetwork] = [networks.optimism, networks.base]
  sourceProvider = optimismProvider
  intentCreator = s.optimismIntentCreator
  solver = s.baseSolver
  console.log(`From ${sourceNetwork.network} to ${destinationNetwork.network}:`)
  await hyperproveInstant()
}
