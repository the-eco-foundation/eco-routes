...
const salt = ethers.keccak256(ethers.toUtf8Bytes('TESTNET-JW14'))
...
async function main() {
  ...
  const intentSourceDeploymentBlockBefore = await ethers.provider.getBlockNumber()
  ...
  try {
    await run('verify:verify', {
      ...
      constructorArguments: [minimumDuration],
