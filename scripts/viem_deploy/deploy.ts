...
>(
  ...
  try {
    ...
    // const code = await (await getPublicClient(chain)).getBytecode({ address: existing })
    // if (code && code.length > '0x'.length) {
    //   console.log(`Contract ${name} already deployed at: ${existing}`)
    //   return existing
    // }
    const { request } = await client.simulateContract({
